...
public interface FriendRepository extends JpaRepository<FriendModel, FriendId> {
	@Query("SELECT f " + "FROM FriendModel f " + "WHERE (f.id.userId = :userId OR f.id.friendId = :userId) "
	...
	@Query("SELECT DISTINCT f " + "FROM FriendModel f " + "JOIN UserModel u1 ON f.id.friendId = u1.id "
			+ "JOIN UserModel u2 ON f.id.userId = u2.id " + "WHERE (f.id.userId = :userId OR f.id.friendId = :userId) "
			+ "AND f.isDelete = false " + "AND (:fullName IS NULL OR u1.fullName LIKE %:fullName%)")
