...
import hcmus.alumni.halloffame.exception.ErrorResponse;
...
public class GlobalExceptionHandler {
	...
	public ResponseEntity<ErrorResponse> handleMissingParams(MissingServletRequestParameterException ex) {
		...
		ErrorResponse error = new ErrorResponse(30000, "Tham số " + name + " bị thiếu");
	...
	public ResponseEntity<ErrorResponse> handleMissingParts(MissingServletRequestPartException ex) {
		...
		ErrorResponse error = new ErrorResponse(30001, "File " + name + " bị thiếu");
	...
	public ResponseEntity<ErrorResponse> handleMaxSizeException(MaxUploadSizeExceededException ex) {
		ErrorResponse error = new ErrorResponse(30002, "Dung lượng file không được vượt quá " + maxFileSize);
	...
	public ResponseEntity<ErrorResponse> handleTypeMismatchParams(MethodArgumentTypeMismatchException ex) {
		...
		ErrorResponse error = new ErrorResponse(30003, "Tham số " + name + " phải là " + requiredType);
	...
	public ResponseEntity<ErrorResponse> handleHttpMessageNotReadableException(HttpMessageNotReadableException ex) {
		ErrorResponse error = new ErrorResponse(30004, "Request body không được để trống");
